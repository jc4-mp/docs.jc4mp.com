...
Make sure to enable abilities once the resource has loaded. In this example, it's using the `OnResourceStart` event to ensure that the resource has fully loaded before enabling the ability. If the ability were enabled outside of this event, it might not trigger properly right when a player joins the server.