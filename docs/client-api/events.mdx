...
#### `OnVehicleLightsUpdate(vehicle: NetVehicle)`

This function is called when the game updates/renders all vehicle lights, one call per vehicle.
...
Event.Add("OnVehicleLightsUpdate", function(vehicle)
    print("Updating lights for vehicle " .. vehicle:GetId())
end)
```
