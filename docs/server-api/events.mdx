...
This fires when a player's resource is started, stopped, or restarted. See [ResourceAction](/shared-api/resources/resourceaction) for more information on types of actions. If you want to send data to a player when they connect, you should do it here since there resource is fully loaded (on ResourceAction.Start).
