...
#### `mods_http_port` (number)

This field is used to specify the port that the server will use to host the mods HTTP server. If this field is not specified, the server will use the port specified in the `port` field + 1.
